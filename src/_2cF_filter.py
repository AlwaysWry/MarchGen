...
def filter_redundant_2cF(_2cF_nonCFds_pool, unlinked_2cF_CFds_pool):
	...
	# on the other hand, the nonCFds*CFds faults cannot be applied to graph-based filter, because only nonCFds member is allowed
	# to be detected, according to the test condition. However, the nonCFds*CFds faults can be self-filtered, because only 1 nonCFds
	# member in the 2cF, so it can be merged into other fault sets directly, and without determining if both members are included by the
	# fault set, like the nonCFds*nonCFds faults.
