...
def generate_2cF_SF_set_dict(_2cF_cover):
	flat_2cF_pool = set()
	inner_sf_pool = {'Init_0': {}, 'Init_1': {}, 'Init_-1': {}}
	inner_sf = TwoComposite()
	for _2cF_obj in _2cF_cover:
...
def filter_redundant_SF(classified_fault_pool, _2cF_cover):
	...
	# faults in 2cF pool get stricter test condition than SF, so use 2cF_pool to filter the current SF pool
	candidate_set_dict_2cF = generate_2cF_SF_set_dict(_2cF_cover)
	...
	for init in filtered_fault_pool.keys():
		...
		for op_num in filtered_fault_pool[init].keys():
			...
			for comp_obj in filtered_fault_pool[init][op_num]:
				...
				identical_flag = find_identical_objs(comp_obj.comps['comp1'], _2cF_cover, {'aCell'})
...
if __name__ == '__main__':
	...
	filter_redundant_SF(copy.deepcopy(classified_pool['SF']), degenerated_2cFs)
