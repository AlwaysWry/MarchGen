...
int MWVC(int narg, char **arg) {
    ...
    string graph_path(arg[0]), out_path(arg[1]);
    double t_max = stod(arg[2]);
    int k = stoi(arg[3]);
    ...
    if (verbose) {
        cout << "QUICK-VC done in " << chrono::duration<double>(tgnn - t1).count() << "s, cost: " << res.cost << endl;
    }
...
PYBIND11_MODULE(QUICK_VC_Py, m){
	...
	for (auto &s: args)
		arg_vec.push_back(const_cast<char*>(s.c_str()));
	return MWVC(int(arg_vec.size()), arg_vec.data());
	});
}
